...
export default function CircuitBoard({ className = "", intensity = "medium" }: CircuitBoardProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      paths.forEach((path, index) => {
        ...
        // Base path
        ctx.strokeStyle = path.type === 'main' ? '#22c55e' : path.type === 'secondary' ? '#16a34a' : '#15803d'
        ...
        const gradient = ctx.createRadialGradient(pulseX, pulseY, 0, pulseX, pulseY, 20)
      ...
      components.forEach((comp) => {
        ...
        switch (comp.type) {
          ...
          case 'capacitor':
            ...
            ctx.fillRect(comp.x + comp.width/2 - 1, comp.y + 2, 2, comp.height - 4)
            break
      ...
      pulseOffset += (isHovered ? 0.02 : 0.01) * (intensity === 'low' ? 0.5 : intensity === 'high' ? 2 : 1)
  ...
  return (
    <div className={`relative w-full h-80 bg-black rounded-lg overflow-hidden border border-green-500/30 ${className}`}>
      <canvas
        ref={canvasRef}
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="text-center p-6 bg-black/80 backdrop-blur-sm rounded-lg border border-green-500/30">
          <motion.h3 
