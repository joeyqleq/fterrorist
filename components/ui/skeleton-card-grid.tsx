...
export default function SkeletonCardGrid({ offers }: SkeletonCardGridProps) {
  ...
  const renderCardBack = (offer: StudentOffer, index: number) => {
    ...
    return (
      <div className="absolute inset-0 bg-black/95 backdrop-blur-xl rounded-md border border-cyan-400/50 p-4 [transform:rotateY(180deg)] [backface-visibility:hidden]">
        <div className="h-full flex flex-col">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
            <h4 className="text-sm font-black text-white uppercase tracking-wider">INTEL</h4>
          </div>

          <div className="space-y-3 mb-4 flex-1">
            <div className="text-sm font-bold text-cyan-400">{offer.provider}</div>
            <div className="text-sm text-gray-300 line-clamp-4">{offer.offer}</div>
              ...
              <div className="absolute inset-0 [backface-visibility:hidden]">
                ...
                `}>
                  ...
                  <div className="relative z-10">
                    ...
                    {variant === "list" && (
                      <div className="mt-auto flex w-full justify-end gap-2">
                        <button className="w-2/5 rounded-sm bg-green-500/80 p-2">
                          <span className="block h-2 rounded-sm bg-black/20" />
                        </button>
                        <button className="w-1/5 rounded-sm bg-muted/80 p-2">
                          <span className="block h-2 rounded-sm bg-gray-300/50" />
                        </button>
                      </div>
                    )}
                    ...
                    {variant === "receipt" && (
                      <span className="mt-auto inline-block w-fit rounded-md bg-gradient-to-r from-cyan-500 to-blue-600 px-3 py-2 text-center text-sm font-semibold text-white">
                        {offer.category}
                      </span>
                    )}
