...
export function BinaryHoverText({
  text,
  className = "",
  ...
  useEffect(() => {
          ...
          if (char !== " " && Math.random() > 0.8) {
            ...
            setTimeout(() => {
              setScrambledChars((prev) => ({
                ...prev,
                [index]: getRandomBinary(),
              }));
              setActiveColors((prev) => ({
                ...prev,
                [index]:
                  binaryColors[Math.floor(Math.random() * binaryColors.length)],
              }));
              ...
              const flickerInterval = setInterval(() => {
                ...
                setActiveColors((prev) => ({
                  ...
                  [index]:
                    binaryColors[
                      Math.floor(Math.random() * binaryColors.length)
                    ],
                }));
                currentFlicker++;
    ...
    ); // Random interval between 800ms - 2s

    return () => clearInterval(interval);
  }, [text]);

  return (
    <span className={className} data-oid="l3_4nqg">
