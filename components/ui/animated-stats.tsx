...
export default function AnimatedStats() {
  ...
  const { freeCount, totalValue, avgDuration } = useMemo(() => {
    ...
    const durations = studentOffers
      .map((offer) => {
        ...
        const months =
          parseInt(num, 10) * (unit.toLowerCase() === "year" ? 12 : 1);
        return months;
      })
  ...
  return (
    <div className="relative mb-12" data-oid="1e2tmz3">
      ...
      >
        <div
          className="relative p-6 rounded-xl border border-green-500/30 bg-black/50 text-center overflow-hidden"
          data-oid="rabag8f"
        >
          <BorderBeam
            size={100}
          ...
          <div
            className="text-gray-400 text-xs mb-1 font-mono"
            data-oid="vm8tp1e"
          >
            Total Student Offers
          </div>
          <Counter
            delay={500}
            direction="up"
